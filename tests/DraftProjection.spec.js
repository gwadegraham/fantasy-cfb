...
const {
    solveQ, expectedCfpPoints, computeLeagueBands, projectTeamPoints,
    buildPoolContext, buildRankingProxy
...
// A remaining-schedule target can exceed the games left to hold it (the standings
// projection subtracts banked wins from a PREseason season total, so losing a
// game leaves the target alone while the games shrink). calibrateToExpectedWins
// then clamps at the game count and hands back a certain win for every game.
describe('projectTeamPoints maxMeanWinProb', () => {
    const season = 2026;
    const teamsById = {
        '1': { id: 1, school: 'Alpha', seasons: [{ season, spRating: 10, expectedWins: 9, conference: 'SEC' }] },
        '2': { id: 2, school: 'Beta', seasons: [{ season, spRating: 0, expectedWins: 6, conference: 'SEC' }] }
    };
    ...
    const games = [1, 2, 3].map(week => ({
        id: 100 + week, season, seasonType: 'regular', week, completed: false,
        conferenceGame: false, neutralSite: false, notes: '',
        homeId: 1, awayId: 2, homeTeam: 'Alpha', awayTeam: 'Beta',
        homeConference: 'SEC', awayConference: 'SEC'
    }));
    // 9 wins wanted from 3 games — the degenerate case.
    const run = (opts) => projectTeamPoints(teamsById['1'], games, poolCtx, rankings, GRAHAM, season,
        Object.assign({ expectedWins: 9, perGame: true }, opts));

    it('without the option, an impossible target makes every game a certain win', () => {
        const probs = run({}).perGame.map(g => g.winProb);
        probs.forEach(p => expect(p).toBeGreaterThan(0.99));
    });

    it('with the option, the mean win probability is held at the ceiling', () => {
        const probs = run({ maxMeanWinProb: 0.9 }).perGame.map(g => g.winProb);
        const mean = probs.reduce((a, b) => a + b, 0) / probs.length;
        expect(mean).toBeLessThanOrEqual(0.9 + 1e-6);
        expect(Math.max(...probs)).toBeLessThan(0.99);
    });

    it('leaves a reachable target alone, so draft grades are unchanged', () => {
        const capped = projectTeamPoints(teamsById['1'], games, poolCtx, rankings, GRAHAM, season,
            { expectedWins: 2, perGame: true, maxMeanWinProb: 0.9 }).perGame.map(g => g.winProb);
        const plain = projectTeamPoints(teamsById['1'], games, poolCtx, rankings, GRAHAM, season,
            { expectedWins: 2, perGame: true }).perGame.map(g => g.winProb);
        expect(capped).toEqual(plain);
    });

    it('never lets a real CFBD pre-game number be capped', () => {
        const withWP = games.map((g, i) => (i === 0 ? Object.assign({}, g, { pregameWinProb: 0.97 }) : g));
        const out = projectTeamPoints(teamsById['1'], withWP, poolCtx, rankings, GRAHAM, season,
            { expectedWins: 9, perGame: true, maxMeanWinProb: 0.9 });
        expect(out.perGame[0].winProb).toBeCloseTo(0.97, 5);
    });
});
